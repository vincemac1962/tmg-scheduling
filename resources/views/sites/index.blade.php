        ...
        @if(count($sites) > 0)
            ...
            <div class="container grid grid-cols-12 gap-4 bg-gray-500 p-4">
                <div class="text-white text-md col-span-1">
                    <a href="{{ route('sites.index', ['sort_by' => 'site_ref', 'direction' => request('sort_by') == 'site_ref' && request('direction') == 'asc' ? 'desc' : 'asc', 'filter' => request('filter')]) }}"
                ...
                <div class="text-white text-md col-span-2">
                    <a href="{{ route('sites.index', ['sort_by' => 'site_name', 'direction' => request('sort_by') == 'site_name' && request('direction') == 'asc' ? 'desc' : 'asc', 'filter' => request('filter')]) }}"
                       class="transititext-primary text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600"
                       data-te-toggle="tooltip"
                       title="Click to sort results by Site Name">Site Name&nbsp;&#9432;</a>
                </div>
                <div class="text-white text-md col-span-4">Site Address</div>
                <div class="text-white text-md col-span-2">Last Contact</div>
                <div class="text-white text-md col-span-2">Last Updated</div>
            ...
            <div class="container grid grid-cols-12 gap-4 p-2">
                ...
                @foreach($sites as $site)
                    ...
                    <div class="col-span-2">
                        {{ Carbon::parse($site->site_last_contact)->format('d/m/Y H:i') }}
                    </div>
                    <div class="col-span-2">
                        {{ Carbon::parse($site->site_last_updated)->format('d/m/Y H:i') }}
                    </div>
                    <div class="col-span-1">
