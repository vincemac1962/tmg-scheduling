...
@section('content')
    <div class="container mx-auto">
            ...
            @foreach($sites as $site)
                <tr>
                    ...
                    <td class="py-2 px-4 border-b">
                        <form action="{{ route('schedules.removeSite', ['schedule' => session('schedule_id'), 'site' => $site->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
        ...
        <a href="{{ route('schedules.show', ['schedule' => session('schedule_id')]) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4">Back</a>
        </div>
    </div>
@endsection
